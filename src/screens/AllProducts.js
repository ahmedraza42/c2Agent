...
//    await getCategories()
  ...
  const getPopularProducts = async () => {
    try {
      const response = await API_CALLS.popularProducts(`?page_no=${1}&Is_view_all=${true}`);
  ...
  const getMoreDetail = async (pageNo = 1) => {
    ...
    console.log({pageNo})
    try {
      const response = await API_CALLS.popularProducts(`?page_no=${pageNo}&Is_view_all=${true}`);
      if (response.status === true) {
  ...
  return (
    ...
    <NormalHeader name={"All Products"} />
